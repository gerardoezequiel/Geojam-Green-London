...
def make_pipeline(model: str = "extra_trees") -> Pipeline:
...
def train_predict(
    ...
    for name, candidate in candidates.items():
        scores = cross_val_score(candidate, X, y, cv=cv, scoring="r2", n_jobs=None)
        scored.append((float(scores.mean()), name, candidate))
    cv_mean, model_name, best = max(scored, key=lambda item: item[0])
    pipe = clone(best)
    pipe.fit(X, y)
    ...
    return ModelResult(out, cv_mean, _importance_table(pipe), time.perf_counter() - started, model_name)
